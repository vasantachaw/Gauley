...
def payment_done(request):
    ...
    for item in cart_items:
        ...
        if item.quantity > product.stock:
            messages.error(request, f"Insufficient stock for {product.title}. Available: {product.stock}")
            continue  # Skip this item, don’t place the order
        ...
        print(f'Order placed: {product.title} x {item.quantity}')
